      ...
      <ul className="p-4 space-y-1 overflow-visible">
        {category.subcategories?.map((sub, subIndex) => (
          <li key={subIndex}>
            ...
            >
              <span className="text-sm text-[#E4D9AF] transition-colors">{sub.name}</span>
              {sub.count && (
                <span className="text-xs px-2 py-1 rounded-full text-[#F9B61D] ">{sub.count}</span>
              )}
            </a>
          </li>
        ))}
...
CategoriaSubCategorias.propTypes = {
